...
- **Never deploy with `--dir .`** — that publishes `Turtle_Nest_Log.xlsx` (the whole nest dataset), `make_nest_report.py` and these docs. `deploy.sh` stages a temp dir with only `index.html`. Verify with a 404 check after deploying.
...
27 nests (25 loggerhead, 1 green, 1 false crawl), 2026-05-01 → 06-29, all with coordinates; 22 photos. Live site current. Backup job verified running unattended. Edit form works (fixed 07-29 — it never had before).
